var kpiScrapSummaryChart3 = function(){
	var days = []; 
	var chart_daily;
	function init(curYear, curMonth){
		//渲染chart1部分
		$.get(manager.root+"/views/tpl/kpi/kpiScrapSummaryChart3.html", function (template) {
			var ractive = new Ractive({
				...
				oncomplete: function(){
					...
					$.ajax({
						url		: manager.root + '/report/scrap/dailyScrapSummaryChart',
						type	: 'GET',
						dataType:"json",
						...
						success: function(listdata)
						{
							...
							chart_daily.addSeries({name:'target',type:'spline',data:_targetList,color:'red'});
